> **Status update:** The legacy server-side lobby-stream endpoint has now been **removed** (not merely "no longer called"), superseding the "no server-side change" non-goal below. Azure Web PubSub (one group per game) is the only real-time transport, and the MAUI lobby now consumes the shared Web PubSub `IGameStateService` described here.

## Why

The MAUI app already has a fully-built, DI-registered shared game-state service (`IGameStateService` over one group-scoped Web PubSub WebSocket), but nothing consumes it. Instead the lobby opened its own separate lobby-stream connection and each play page (prey, hunter) opened its own separate Web PubSub socket — so moving from the lobby into gameplay tore down one connection and opened another, with a gap in live updates during the handoff and three parallel implementations of the same connect/join/backoff logic to maintain. We want one connection, opened at the lobby, that stays alive across the navigation into gameplay and feeds every page.
...
- Rewire `GameLobbyViewModel` to render each `GameDetails` snapshot broadcast by the shared service instead of consuming its own separate lobby stream (`ILobbyStreamClient`).
...
- **Backend**: the Web PubSub token endpoint (`GET /games/{id}/notifications/token`), the group-broadcast events, and `GET /games/{id}/status` are unchanged. The backend lobby-stream endpoint the MAUI client used has since been removed.
