---
title: "Privacyverklaring"
description: "Hoe The Prey jouw persoonsgegevens verzamelt, gebruikt en beschermt — waaronder locatiegegevens, accountgegevens en spelactiviteit."
keywords: ["The Prey privacy", "privacybeleid", "AVG", "GDPR", "locatiegegevens", "gegevensbescherming", "persoonsgegevens"]
---

_Laatst bijgewerkt: 16 juni 2026_

The Prey is een op GPS gebaseerd verstoppertjesspel. Omdat het spel de locatie van spelers in realtime volgt, nemen we jouw privacy serieus. In deze verklaring lees je welke gegevens we verzamelen, waarom we dat doen en welke rechten je hebt op grond van de Algemene Verordening Gegevensbescherming (AVG).

---

## Wie Wij Zijn

The Prey wordt vanuit Nederland beheerd en is bereikbaar via **theprey.nl**. Voor privacygerelateerde vragen of verzoeken kun je contact opnemen via **privacy@theprey.nl**.

---

## Welke Gegevens Wij Verzamelen

| Categorie | Voorbeelden | Waarom |
|---|---|---|
| Accountgegevens | Naam of bijnaam, e-mailadres, authenticatie-identificatie | Om je account aan te maken en te beveiligen |
| Locatiegegevens | GPS-coördinaten tijdens een actief spel | Kern van het spel — het volgen van Prooi en Jagers |
| Spelgegevens | Speelvelden die je maakt, spellen die je speelt, rollen, tags, resultaten | Om spellen te draaien en terug te kijken |
| Technische gegevens | Apparaattype, app-versie, diagnostische logs | Om de dienst te beheren, beveiligen en verbeteren |

We **verkopen** jouw persoonsgegevens niet en gebruiken ze niet voor advertenties.

---

## Locatiegegevens

Locatie is het hart van The Prey en verdient daarom een aparte vermelding:

- GPS-coördinaten worden **alleen verzameld terwijl je deelneemt aan een actief spel**.
- Locaties worden uitsluitend met andere spelers gedeeld volgens de spelregels (zo worden Prooi-posities alleen tijdens de Actieve Jacht aan Jagers getoond).
- Zodra een spel eindigt, stopt het delen van live locaties.
- We volgen jouw locatie niet op de achtergrond buiten een actief spel.

---

## Waarom Wij Jouw Gegevens Verwerken

We verwerken jouw gegevens om:

- Je te authenticeren en je account veilig te houden.
- Spellen te draaien, inclusief realtime locatievolging en meldingen.
- De speelvelden en spelgeschiedenis te bewaren die nodig zijn om spellen terug te kijken en te verbeteren.
- De beveiliging, stabiliteit en prestaties van de dienst te waarborgen.

Onze grondslagen zijn de **uitvoering van een overeenkomst** (het aanbieden van het spel dat je wilt spelen) en ons **gerechtvaardigd belang** bij het beheren van een veilige, betrouwbare dienst.

---

## Met Wie Wij Gegevens Delen

We delen gegevens alleen met dienstverleners die ons helpen The Prey te draaien:

- **Authenticatie** — onze identiteitsprovider (Auth0) verzorgt het veilig inloggen.
- **Cloudhosting** — onze backend draait op Microsoft Azure binnen de Europese Unie.
- **Andere spelers** — tijdens een spel is beperkte spelinformatie (zoals je rol en, waar de regels dit toestaan, je locatie) zichtbaar voor andere deelnemers.

Deze partijen verwerken gegevens namens ons op basis van overeenkomsten die passende waarborgen vereisen.

---

## Hoe Lang Wij Gegevens Bewaren

- **Accountgegevens** bewaren we zolang je account bestaat.
- **Locatiegegevens** uit spellen bewaren we alleen zo lang als nodig is om een spel te leveren en terug te kijken, daarna verwijderen of anonimiseren we ze.
- **Diagnostische logs** bewaren we voor een beperkte periode voor beveiliging en probleemoplossing.

Wanneer je je account verwijdert, verwijderen of anonimiseren we je persoonsgegevens, behalve waar we deze wettelijk verplicht moeten bewaren.

---

## Jouw Rechten

Op grond van de AVG heb je het recht om:

- De persoonsgegevens die wij over je hebben in te zien.
- Onjuiste of onvolledige gegevens te corrigeren.
- Je gegevens te laten verwijderen ("recht op vergetelheid").
- Bepaalde verwerkingen te beperken of er bezwaar tegen te maken.
- Je gegevens in een overdraagbaar formaat te ontvangen.

Om een van deze rechten uit te oefenen, mail je naar **privacy@theprey.nl**. Je hebt ook het recht om een klacht in te dienen bij de Autoriteit Persoonsgegevens.

---

## Cookies

Deze website gebruikt alleen de cookies en lokale opslag die nodig zijn om de site en je aanmelding te laten werken. We gebruiken geen tracking- of advertentiecookies.

---

## Wijzigingen in Deze Verklaring

We kunnen deze privacyverklaring van tijd tot tijd bijwerken. De datum bovenaan geeft de meest recente wijziging weer. Belangrijke wijzigingen communiceren we via de app of website.

---

## Contact

Vragen over je privacy? Mail ons via **privacy@theprey.nl**.
