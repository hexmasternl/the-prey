...
### Requirement: participant-located event emitted when hunter location is updated

When the hunter records a location, the system SHALL emit a `participant-located` SSE event to all connected prey participants. The event data SHALL include `participantRole` ("Hunter"), `latitude`, and `longitude`. Prey-to-prey location data SHALL NOT be broadcast to avoid information leakage.
...
- **THEN** each connected prey participant receives a `participant-located` event carrying the hunter's coordinates

#### Scenario: Prey location not broadcast to other preys

- **WHEN** a prey participant records a GPS location
- **THEN** no `participant-located` event is emitted to other prey clients
