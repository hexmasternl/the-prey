# maui-game-hud-tag Specification

## Purpose
Define the hunter-only Tag flow surfaced on the in-game HUD: the Tag button's hunter-only visibility, fetching the preys currently within tag range, a modal selection dialog listing candidates, a confirmation gate, the tag round-trip, and the empty / rejected / unauthorized outcomes.
