...
The reusable client seams already exist from the hunter change: `IGameApiClient.GetGameStatusAsync` + `GameStatusDetails`, `IGameStreamClient` (Web PubSub) + `GameStreamEvent`, `ILivePositionReader`, `IHeadingReader`, the shared head-start countdown overlay, and the outcome-hand-off navigator; plus `IAccessTokenProvider`, `TimeProvider`, the Mapsui `MapControl` usage, the localization service + `{loc:Translate}`, and the single-source Colors/Styles. The Angular `game-prey.page.ts` is the authoritative UX reference (status poll seeds all blips; the stream pushes the hunter live; hunter red, other preys colored + grey when tagged; self green arrow; spectator on self-tag).
...
- The shared **head-start countdown** to `HunterMayMoveAt` **without** the move-early / penalty warning; the map stays readable while it counts down.
...
### D3: Same four-phase machine; the prey head-start overlay omits the warning and does not block the map
`Phase` ∈ { `Waiting`, `HeadStart`, `Live`, `Ended` } is derived from `GameDto.Status` and `HunterMayMoveAt` exactly as in the hunter VM. The difference is the **HeadStart** presentation: the prey sees the shared countdown overlay with the move-early warning **hidden**, and the overlay is non-blocking so the prey can read and pan the map to hide while the hunter is still frozen. Resuming after `HunterMayMoveAt` has passed enters **Live** directly; a `Completed` game hands off immediately.

- **Why:** the head start is the window in which preys hide before the hunter is released; the prey needs the countdown (how long to hide) but not the hunter's "don't move" penalty. This fulfils the hunter change's note that the countdown overlay is the shared control reused with the warning hidden.
