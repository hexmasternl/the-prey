# maui-game-lobby-share Specification

## Purpose
Provide the lobby's pass-code display and Share action — constructing the localized invite text and the join deep link from the game code, and handing it to the device's native share sheet, including the no-op when the user dismisses the share sheet.
