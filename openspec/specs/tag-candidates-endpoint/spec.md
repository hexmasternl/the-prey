# tag-candidates-endpoint Specification

## Purpose
Defines the server-side endpoint that returns the preys currently within tagging range of the hunter, so the hunter client can present only taggable candidates.
