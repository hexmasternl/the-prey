# design-system-compliance Specification

## Purpose

The normative rules every front-end screen in `src/ThePrey` must follow to conform to The Prey Design System v1.0 — color tokens, typography, spacing/radius, component patterns, role-color semantics, themed overlays, motion/accessibility, and tactical copy — plus per-area conformance requirements for playfields, games, and shell/auth screens.
